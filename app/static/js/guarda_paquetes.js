...
document.getElementById("pkg-form").addEventListener("submit", async e => {
  ...
  if (pkgEsTercero.checked) {
    ...
    if (r.ok && j.ok) {
      ...
      guiarA(okBox, true);
      if (j.package.tercero_celular) {
        document.getElementById("btn-wa-tercero").addEventListener("click", () => enviarPaseWhatsapp(j.qr_data_uri, texto, "paquete-vie-" + j.package.short_code + ".png"));
      }
    } else {
...
if (pkgPaseCard) {
  document.querySelectorAll("[data-verqr-pkg]").forEach(btn => btn.addEventListener("click", async () => {
    ...
    if (btn.dataset.cel) {
      ...
      wa.onclick = () => enviarPaseWhatsapp(j.qr_data_uri, texto, "paquete-vie-" + p.short_code + ".png");
    } else {
